...
class SainstaController extends Controller
{
    public function index()
    {
        ...
        $instanciaspadre = Sainsta::where('comercial',$comercial)->selectRaw(" CONCAT (repeat('&nbsp;',(Nivel)*4) , ' ' ,  Descrip) as label, descrip, id ")->orderBy('codalte')->get();
    ...
    public function lastprod($codinst)
    {
        $last   = '';
        $comercial  = session('comercialid') ;
        $product = Saprod::where(['comercial'=> $comercial, 'codinst'=>$codinst])
